...
def parse_page(htmlstring, driver, driver1):
    ...
    time.sleep(3)
    driver.get("https://www.quora.com/topic/Accounting/top_questions")
    ...
    while (flag):
        ...
        if i >= 0:
            try:
                ...
                question_url = driver.find_element_by_xpath(("//div[contains(@class, 'paged_list_wrapper')]/div[{}]//a[@class='question_link']").format(i + 1)).get_attribute('href')
            except:
                flag = False
                print("---------------------THE END-------------------------")    
                return
        ...
        if i % 10 == 1:
            print("*********************************************************************&&******************")
            try:
                scroll_target = driver.find_element_by_xpath("//div[contains(@class, 'paged_list_wrapper')]/div[{}]".format(i + 9))
                print(scroll_target)
...
csvfile = open("quora_business_top.csv", "wb")
